...
class RegisteredUser(User):
    ...
    def calculate_bmr(self):
        ...
        if self.sex.upper() == "M":
            self.bmr = round(66.5 + (13.75 * self.weight) + (5.003 * height_m) - (6.75 * self.age), 2)
        elif self.sex.upper() == "F":
            self.bmr = round(655.1 + (9.563 * self.weight) + (1.850 * height_m) - (4.676 * self.age), 2)
...
def save_user_data(users):
    with open("users.txt", "w") as file:
        for user in users:
            user.calculate_bmr()
            file.write(f"{user.username}:{user.password}:{user.age}:{user.weight}:{user.height}:{user.sex}:{user.bmr}:0\n")
    ...
    try:
        with open("users.txt", "r") as file:
            ...
            for line in file:
                ...
                if len(fields) == 7:  # Adjusted the condition for BMR field
                    username, password, age, weight, height, sex, bmr = fields
